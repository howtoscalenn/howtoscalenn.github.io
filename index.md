...
In this regime, the main questions are:
...
Compared to muP,  
`Standard Parameterization (SP)` (e.g. He, Lecun, and other default PyTorch initializations) is not designed for scaling.  
These methods are well-defined `only at initialization, not during training`.  
That means they don't account for how weight updates affect model behavior—leading to what’s known as "left-shifted lr curves."

But our goal is to define a parameterization in which optimal behavior transfers as model size scales. That means we need to properly set per-layer:

- lr
- init std
- multiplier  

And that’s `one of the core desiderata of muP`.
...
If we define this scaling rule properly,  
we can efficiently tune larger models—and match scaling laws—at relatively low cost.
...
muP is designed for 'maximal feature learning'.  
But to achieve this, you also need to maintain 'training stability'.

If you increase the lr to ensure that every weight receives a sufficiently large update,  
some weights might receive *too much* gradient and cause divergence.  
On the other hand, reducing the lr to stabilize training can prevent some layers from evolving at all—especially if you use a single global lr across all parameters.  
And this can lead to inefficient training.

That’s why we need to analyze 'per-layer behavior' and adopt 'per-layer parameterization' (i.e., per-layer lr, init std, and multiplier), as done in muP.
...
So even if your NTK-parameterized BERT is pretrained successfully,  
its learned representation is weak.  
If you tried to concatenate a randomly initialized linear layer to its hidden features and fine-tune,  
it wouldn’t work, because the hidden layers never learned meaningful features.
...
- (Very) roughly speaking many techniques like LayerNorm, BatchNorm, and adaptive optimizers are all motivated by this.
...
- Before residual additions
- Mixing post-norm and pre-norm configurations
- ...
...
where the authors report that QK norm enlarges the lr basin.  
...
But as we discussed earlier, muP doesn't guarantee HP transfer across training horizon or bsz.  
So using QK norm can be a good practical choice because it can increase the probability of choosing near-optimal lr.  
...
I would like to thank [Jingyuan Liu](https://x.com/JingyuanLiu123) for reading this post in advance and sharing insightful feedback, especially on MoE scaling factors.
